...
sys.path.append('/Users/schwenk/Library/Jupyter/extensions')
...
def construct_output_py_file_path(input_file_path, skip_if_exists=True):
    py_out_dir = 'record_notebook_py'
    input_headless, ext = os.path.splitext(input_file_path)
    assert ext=='.ipynb'
    output_dir_base, filename = '/'.join(input_headless.split('/')[:-1]), input_headless.split('/')[-1]
    output_dir = os.path.join(output_dir_base, py_out_dir)
    try:
        os.mkdir(output_dir)
    except FileExistsError:
        pass
    output_by_path = os.path.join(output_dir, filename + '.py')
    if os.path.exists(output_by_path) and skip_if_exists:
        return
    return output_by_path

def construct_output_html_file_path(input_file_path, skip_if_exists=True):
    html_out_dir = 'record_notebook_html'
    input_headless, ext = os.path.splitext(input_file_path)
    assert ext=='.ipynb'
    output_dir_base, filename = '/'.join(input_headless.split('/')[:-1]), input_headless.split('/')[-1]
    output_dir = os.path.join(output_dir_base, html_out_dir)
    try:
        os.mkdir(output_dir)
    except FileExistsError:
        pass
...
# c.NotebookApp.notebook_dir = u'/Users/schwenk/Metis/Projects/Luther/challenges'
...
# 
# c.MappingKernelManager.root_dir = u'/Users/schwenk/Metis/Projects/Luther/challenges'
...
# 
# c.FileNotebookManager.notebook_dir = u'/Users/schwenk/Metis/Projects/Luther/challenges'
